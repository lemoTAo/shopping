          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for=" spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                @click="changeActive(spuSaleAttr.spuSaleAttrValueList,spuSaleAttrValue)">{{spuSaleAttrValue.saleAttrValueName}}</dd>
                
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model.number="num" @blur="changeNum" oninput = "value=value.replace(/[^\d]/g,'')">
                <a href="javascript:" class="plus" @click="num++">+</a>
                <a href="javascript:" class="mins" @click="num > 1? num-- : num=1">-</a>
              </div>
              <div class="add">
                <!-- 这里路由跳转从A调到B发送了请求，之前是获取请求 -->
    ...
    methods:{
      ...
      changeNum(){
       ...
       addShopCar(){
        ...
        //2.请求成功，存储，跳转
        //3.失败，给予用户提示    
        //查看是否失败，配置配置层一个promise对象,promise对象可以配置成败回调
        //请求成功进行路由的跳转
      const result =this.$store.dispatch('detail/addOrUpdateShopCar',{skuId:this.$route.params.skuId,skuNum:this.num})
      result.then(()=>{
        //参数携带 ,携带的参数过大、复杂地址栏一大栏，采用会话存储（不持久化，会话结束就消失）本地存储（持续保留）他们只能存储JSON对象，简单采用query传递
        sessionStorage.setItem('item',JSON.stringify(this.skuInfo))
        this.$router.push({
          name:'AddCartSuccess',
